...
Server.command.register(registerInformation, (chatmsg, args) => {
    ...
    if (sender.hasTag('admin')) {
        if (worldset.includes(args[0])) {
            ...
            sender.tellraw(`§¶§7[§eConsole§7]§f ► §b§lWorld Spawn configured to §e${sender.scoreTest('Worldx')} ${sender.scoreTest('Worldy')} ${sender.scoreTest('Worldz')}§b! Players will be sent here after passing World Border`);
        }
            ...
            sender.tellraw(`§¶§7[§eConsole§7]§f ► §b§lCustom World Spawn has been set back to default`);
        }
        else {
            sender.tellraw(`§¶§7[§eConsole§7]§f ► §cERROR 2! §6Usage Example §7:§b§l UAC.worldspawn [ set | remove ]\n§¶§cINFO ► §bThis tells UAC where worldspawn is. Where you're standing will also be where default spawn is now. People will teleport here when crossing world border, or when using the "Spawntp" chat command.`);
        }
    } else {
        sender.tellraw(`§¶§7[§eConsole§7]§f ► §c§lError 4: Only Staff can configure world spawn`);
    }
});