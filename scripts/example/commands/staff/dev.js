    ...
    try {
        ...
        if (sender.hasTag('admin')) {
            if (devinit.includes(args[0])) {
                if(args[1]) {
                    if (!playerfound) {
                        return sender.tellraw(`§¶§7[§eConsole§7]§f ► §c§lError 7: No player by that name`); 
                    }
                    else { 
                        let playername = playerfound.getName();
                        sender.tellraw(`§¶§7[§eConsole§7]§f ► §d§l${playername}'s §binit was reset by §d${name}`);
                ...
                return sender.tellraw(`§¶§7[§eConsole§7]§f ► §cNo args given`);
            }
        } else {
