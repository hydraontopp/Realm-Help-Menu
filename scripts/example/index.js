...
world.events.tick.subscribe(({ deltaTime, currentTick }) => {
    try {
        ...
        const acmbool = scoreTest('togglecommands', 'togglecommands');
        ...
        for (let player of players) {                                                                     //scorecheck for vanilla api
            ...
            for (let i = 0; i < playerInventory.size; i++) {
                ...
                if(player.hasTag('admin')) { return } else {
            ...
            if(player.hasTag('admin')) { return } else {
                if (acmbool && itemArray.length) {
                    overworld.runCommand(`tellraw @a {"rawtext":[{"text":"§¶§7[§eConsole§7]§f ► §6Anti-CBE §d${name} §bwas temp-kicked for having §c${itemArray}"}]}`);
                    player.runCommand('function utils/asset/cbeitem_gt_warn');
                }
            }
        }
    } catch (error) {
...
// credit to uac, we use there updated gametest!
